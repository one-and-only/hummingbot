...
from hummingbot.connector.exchange.aax import aax_constants as CONSTANTS, aax_utils, aax_web_utils as web_utils
from hummingbot.connector.exchange.aax.aax_api_order_book_data_source import AAXAPIOrderBookDataSource
from hummingbot.connector.exchange.aax.aax_api_user_stream_data_source import AAXAPIUserStreamDataSource
from hummingbot.connector.exchange.aax.aax_auth import AAXAuth
...
class AAXExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self.api_key = aax_api_key
        self.secret_key = aax_api_secret
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_aax_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def aax_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(aax_type: str) -> OrderType:
        return OrderType[aax_type]
    ...
    async def get_all_pairs_prices(self) -> List[Dict[str, str]]:
        ...
        for pair in self.trading_pairs:
            order_book = await self._api_get(path_url=f"/market/orderbook?symbol={pair}&level=20")
            pairs_prices.append(order_book)
            pairs_prices
        return pairs_prices
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return AAXAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return AAXAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        price_str = f"{price:f}"
        type_str = AAXExchange.aax_order_type(order_type)
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        cancel_result = await self._api_delete(
            path_url=f"/spot/orders/cancel/{order_id}",
            params={},
            is_auth_required=True)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(aax_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_aax_timestamp * 1e3)
            self._last_trades_poll_aax_timestamp = self._time_synchronizer.time()
    ...
    async def _update_balances(self):
        ...
        for balance_entry in balances:
            ...
            total_balance = Decimal(balance_entry["available"]) + Decimal(balance_entry["unavailable"])
            self._account_available_balances[asset_name] = free_balance
            self._account_balances[asset_name] = total_balance
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(aax_utils.is_exchange_information_valid, exchange_info["symbols"]):
